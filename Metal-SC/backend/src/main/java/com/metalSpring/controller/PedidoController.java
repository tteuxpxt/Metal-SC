...
public class PedidoController {
    ...
    private PedidoDTO toPedidoDTO(Pedido pedido) {
        ...
        dto.setValorFinalNegociado(pedido.getValorFinalNegociado());
        dto.setStatus(pedido.getStatus());
        dto.setStatusPagamento(pedido.getStatusPagamento());
        dto.setAprovacaoCliente(pedido.getAprovacaoCliente());
        dto.setAprovacaoRevendedor(pedido.getAprovacaoRevendedor());
        dto.setDataCriacao(pedido.getDataCriacao());
        dto.setDataAprovacaoCliente(pedido.getDataAprovacaoCliente());
        dto.setDataAprovacaoRevendedor(pedido.getDataAprovacaoRevendedor());
        dto.setDataPagamentoInformadoCliente(pedido.getDataPagamentoInformadoCliente());
        dto.setDataPagamentoConfirmado(pedido.getDataPagamentoConfirmado());
        dto.setHistoricoEventos(pedido.getHistoricoEventos());
    ...
    public ResponseEntity<?> criar(@RequestBody PedidoDTO dto) {
        try {
            ...
            if (atualizado.getItens() != null && !atualizado.getItens().isEmpty()) {
                negociacaoService.iniciarPorPedido(atualizado);
                atualizado = pedidoService.buscarPorId(pedido.getId()).orElse(atualizado);
            }
    ...
    @PostMapping("/{id}/liberar-pagamento")
    public ResponseEntity<?> liberarPagamento(@PathVariable String id) {
        try {
            Pedido pedido = pedidoService.liberarPagamento(id);
            return ResponseEntity.ok(toPedidoDTO(pedido));
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(java.util.Map.of("error", e.getMessage()));
        }
    ...
    @PostMapping("/{id}/informar-pagamento")
    public ResponseEntity<?> informarPagamento(@PathVariable String id, @RequestParam(required = false) String clienteId) {
        try {
            Pedido pedido = pedidoService.informarPagamento(id, clienteId);
            return ResponseEntity.ok(toPedidoDTO(pedido));
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(java.util.Map.of("error", e.getMessage()));
        }
