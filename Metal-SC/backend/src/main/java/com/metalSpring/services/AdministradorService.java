...
public class AdministradorService {
    ...
    public Administrador criar(Administrador admin) {
        ...
        if (admin.getSenhaHash() != null && !admin.getSenhaHash().startsWith("$2")) {
    ...
    public Map<String, Object> gerarDashboard() {
        ...
        vendas.put("aguardandoNegociacao", pedidoRepository.countByStatus(PedidoStatus.AGUARDANDO_NEGOCIACAO));
        vendas.put("pagamentoInformado", pedidoRepository.countByStatus(PedidoStatus.PAGAMENTO_INFORMADO_CLIENTE));
        List<Pedido> divergencias = pedidoRepository.findByStatusAndDataPagamentoInformadoClienteBefore(
                PedidoStatus.PAGAMENTO_INFORMADO_CLIENTE,
                LocalDateTime.now().minusHours(24)
        );
        vendas.put("divergenciasPagamento", divergencias.size());
        vendas.put("divergenciasPagamentoDetalhes", divergencias.stream()
                .map(pedido -> Map.of(
                        "pedidoId", pedido.getId(),
                        "clienteNome", pedido.getCliente() != null ? pedido.getCliente().getNome() : "Cliente",
                        "revendedorNome", pedido.getVendedor() != null ? pedido.getVendedor().getNome() : "Revendedor",
                        "informadoEm", pedido.getDataPagamentoInformadoCliente(),
                        "valor", pedido.getValorFinalNegociado() != null ? pedido.getValorFinalNegociado() : pedido.getValorTotal()
                ))
                .toList());
